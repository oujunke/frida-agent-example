...
var addresses:any = {};
var SSL_get_fd:any = null;
var SSL_get_session:any = null;
var SSL_SESSION_get_id:any = null;
var getpeername = null;
var getsockname = null;
var ntohs = null;
var ntohl:any = null;
var SSLstackwrite:any = null;
var SSLstackread:any = null;
...
function sendMsg(msg:any,data:any){
    console.log(data);
    //console.log(JSON.stringify(msg)+"::"+data);//+Base64.encode(data)
}
/**
 ...
 *     and "dst_port".
 */
function getPortsAndAddresses(sockfd:any, isRead:any) {
    var message:any = {};
    var src_dst:any = ["src", "dst"];
    for (var i = 0; i < src_dst.length; i++) {
        if (("src"==src_dst[i]?1:0) ^ isRead) {
        var sockAddr:any = Socket.localAddress(sockfd)
        }
        else {
        var sockAddr:any = Socket.peerAddress(sockfd)
        }
        if (sockAddr == null) {
        ...
        message[src_dst[i] + "_addr"] = sockAddr.ip;//ntohl(ipToNumber(sockAddr.ip.split(":").pop()))
        console.log(sockAddr.ip)
        }
    }
    return message;
}
 ...
 */
function getSslSessionId(ssl:any) {
    var session = SSL_get_session(ssl);
    if (session == 0) {
        return 0;
    }
    var lenP = Memory.alloc(4);
    var p = SSL_SESSION_get_id(session, lenP);
...
export function start(){
    ...
    Interceptor.attach(addresses["SSL_read"],
    {
        ...
        sendMsg(this.message, this.buf.readCString(retval));
        }
    });
    ...
    Interceptor.attach(addresses["SSL_write"],
    {
        ...
        sendMsg(message, args[1].readCString(args[2].toInt32()));
        },
    ...
    if (Java.available) {
        Java.perform(function () {
            function storeP12(pri:any, p7:any, p12Path:any, p12Password:any) {
            ...
            sendMsg(message, data.readCString(1))
            return result;
            }
            ...
            Java.use("java.net.SocketOutputStream").socketWrite0.overload('java.io.FileDescriptor', '[B', 'int', 'int').implementation = function (fd:any, bytearry:any, offset:any, byteCount:any) {
            var result = this.socketWrite0(fd, bytearry, offset, byteCount);
            var message:any = {};
            message["function"] = "HTTP_send";
            message["ssl_session_id"] = "";
            message["src_addr"] =this.socket.value.getLocalAddress().toString();// ntohl(ipToNumber((this.socket.value.getLocalAddress().toString().split(":")[0]).split("/").pop()));
            message["src_port"] = parseInt(this.socket.value.getLocalPort().toString());
            message["dst_addr"] = this.socket.value.getRemoteSocketAddress().toString();//ntohl(ipToNumber((this.socket.value.getRemoteSocketAddress().toString().split(":")[0]).split("/").pop()));
            ...
            for (var i = 0; i < byteCount; ++i)
                ptr.add(i).writeS8(bytearry[offset + i]);
            sendMsg(message, ptr.readCString(byteCount))
            return result;
            }
            Java.use("java.net.SocketInputStream").socketRead0.overload('java.io.FileDescriptor', '[B', 'int', 'int', 'int').implementation = function (fd:any, bytearry:any, offset:any, byteCount:any, timeout:any) {
            ...
            if (result > 0) {
                ...
                for (var i = 0; i < result; ++i)
                    ptr.add(i).writeS8(bytearry[offset + i]);
                sendMsg(message, ptr.readCString(result))
            }
            return result;
            }

            if (parseFloat(Java.androidVersion)  > 8) {
            Java.use("com.android.org.conscrypt.ConscryptFileDescriptorSocket$SSLOutputStream").write.overload('[B', 'int', 'int').implementation = function (bytearry:any, int1:any, int2:any) {
                var result = this.write(bytearry, int1, int2);
                SSLstackwrite = Java.use("android.util.Log").getStackTraceString(Java.use("java.lang.Throwable").$new()).toString();
                return result;
            }
            Java.use("com.android.org.conscrypt.ConscryptFileDescriptorSocket$SSLInputStream").read.overload('[B', 'int', 'int').implementation = function (bytearry:any, int1:any, int2:any) {
            ...
            Java.use("com.android.org.conscrypt.OpenSSLSocketImpl$SSLOutputStream").write.overload('[B', 'int', 'int').implementation = function (bytearry:any, int1:any, int2:any) {
                var result = this.write(bytearry, int1, int2);
                SSLstackwrite = Java.use("android.util.Log").getStackTraceString(Java.use("java.lang.Throwable").$new()).toString();
                return result;
            }
            Java.use("com.android.org.conscrypt.OpenSSLSocketImpl$SSLInputStream").read.overload('[B', 'int', 'int').implementation = function (bytearry:any, int1:any, int2:any) {
                var result = this.read(bytearry, int1, int2);
                SSLstackread = Java.use("android.util.Log").getStackTraceString(Java.use("java.lang.Throwable").$new()).toString();
                return result;
            }
